...
from sklearn.naive_bayes import BernoulliNB #use bernoulli naive bayes in sklearn
clf = BernoulliNB()
...
with open('chchan2_3.csv','wt') as f:
...
for i in range(10):
    ...
    imsave(name = 'unt_bern_'+str(i)+'.jpg', arr = plot_mean_i)

clf = BernoulliNB()
clf.fit(rc_train_fea, train_label)
test_label = clf.predict(rc_test_fea)
test_label = np.asarray(test_label, dtype = np.int32)
with open('chchan2_4.csv','wt') as f:
    f.write('ImageId,Label')
    for i in range(test_label.shape[0]):
        f.write('\n' + str(i) + ',' + str(test_label[i]))
f.close()
...
for i in range(10):
    ...
    imsave(name = 'str_bern_'+str(i)+'.jpg', arr = plot_mean_i)
