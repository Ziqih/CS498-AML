...
(fea,label,sam_size,fea_size)=read_data('pima-indians-diabetes.csv') #self-defined function that generates features and labels
### random split: pick a random number in the number of data. If it is within the first 80% of the data, pick 20% data after the number for testing; otherwise, pick 20% data before the number for testing. Repeat the process 10 times ###
...
for i in range(10):
    ...
    for t in range(test_start[i],test_end[i]):
        ...
        for f in range(fea_size): #calculate the sum of logarithmic normal distribution probability for each class
            if (abs(pos_mean[f]-fea[t,f]) <= (3*sqrt(pos_var[f]))) or (abs(neg_mean[f]-fea[t,f]) <= (3*sqrt(neg_var[f]))):#exclude fea_sizeture data greater than 3 variance to improve accuracy
